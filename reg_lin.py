...
def linear_regression(prob: Prob, data: Data) -> Tuple[float, float, float]:
...
def test_linear_regression(model: Callable[[Prob, Data], Tuple[float, float, float]], data: Data, \
     method: Type[InferenceMethod[Data, Tuple[float, float, float]]], model_name: str, n: int = 1000,
    ...
    if only_best_values is not None:
        a = float(np.average([r[0][0] for r in sorted_res[:only_best_values]], 
                             weights=[r[1] for r in sorted_res[:only_best_values]]))
        b = float(np.average([r[0][1] for r in sorted_res[:only_best_values]],
                             weights=[r[1] for r in sorted_res[:only_best_values]]))
    else:
        a = float(np.average([r[0][0] for r in sorted_res], 
                               weights=[r[1] for r in sorted_res]))
        b = float(np.average([r[0][1] for r in sorted_res],
                             weights=[r[1] for r in sorted_res]))
